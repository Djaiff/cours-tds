...
- Imaginons maintenant que l'on dispose d'un échantillon de $\N$ observations que l'on suppose issues d'une loi normale $\Norm(\mu, \sigma^2)$

Comment faire pour estimer la vraie moyenne $\mu$ avec les données dont on dispose ?

## Intervalle de confiance: intérêt

Une première intuition peut consister à supposer que la vraie moyenne $\mu$ est \og à peu près\fg{} égale à la moyenne de l'échantillon dite empirique i.e., $\bar \x_\N$

$$\text{Estimation ponctuelle : } \mu \sim \bar \x_\N = \frac{1}{\N}\sum_{i=1}^{\N}\x_i$$
...
Lorsqu'on dispose d'observation d'une distribution supposée gaussienne (disons $\Norm(\mu, \sigma^2)$), on dispose de la vraie loi de la moyenne empirique:

$$\bar \X_\N \sim \Norm(\mu, \frac{\sigma^2}{\N})$$

Ainsi en normalisant (centrage+réduction), on peut déduire que:

$$\sqrt \N \frac{\bar \X_\N -\mu}{\sigma} = \frac{\bar \X_\N -\mu}{\frac{\sigma}{\sqrt \N}}  \sim \Norm(0, 1)$$

## Rappel sur les quantiles

En reprenant les notation des quantiles de la loi normale i.e. si $\Z\sim \Norm(0,1)$ et $\alpha \in ]0.5;1[$, $\z_{1-\alpha/2}$ vérifie
...
$$P(-\z_{(1-\alpha/2)} \leq \sqrt \N \frac{\bar \X_\N -\mu}{\sigma} \leq \z_{(1-\alpha/2)}) = 1-\alpha$$

Ce qui est équvalent à écrire que:

$$P(\bar \X_\N - \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)} \leq \mu \leq \bar \X_\N + \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)}) = 1 - \alpha$$
...
$$P(\mu \in [\bar \X_\N - \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)}; \bar \X_\N + \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)}]) = 1-\alpha$$
...
$$[\bar \x_\N - \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)}; \bar \x_\N  + \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)}]$$
...
$$\sigma \quad\text{estimé par}\quad s=\sqrt{\frac{1}{\N-1}\sum_{i=1}^\N (\x_i-\bar \x_\N )^2} = \sqrt{\frac{\N}{\N-1} \sigma_{\mathrm{obs}}^2}$$
...
$$[\bar \x_\N  - \frac{\s}{\sqrt \N} t_{(1-\alpha/2)}; \bar \x_\N  + \frac{\s}{\sqrt \N} t_{(1-\alpha/2)}]$$
...
$$
\begin{array}{rl}
&[\bar \x_\N  - \frac{\sigma}{\sqrt \N} t_{(1-\alpha/2)}; \bar \x_\N  + \frac{\sigma}{\sqrt \N} t_{(1-\alpha/2)}]\\
\simeq& [\bar \x_\N  - \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)}; \bar \x_\N  + \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)}]
\end{array}
$$
...
L'intervalle de confiance $[\bar \x_\N  - \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)}; \bar \x_\N  + \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)}]$ est donc symétrique, centré sur $\bar \x_\N$. On écrit d'ailleurs souvent par abus de notation:

$$[\bar \x_\N  \pm \frac{\sigma}{\sqrt \N} \z_{(1-\alpha/2)}] \quad \text{ou}\quad [\bar \x_\N  \pm \frac{s}{\sqrt \N} t_{(1-\alpha/2)}]$$
...
$$P \underset{n\to \infty}{\longrightarrow} \Norm(\mu=p_0,\sigma^2=\frac{p_0(1-p_0)}{\N})$$
